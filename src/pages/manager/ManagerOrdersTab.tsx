  ...
  const loadOrders = useCallback(
    async (isManualRefresh = false) => {
      ...
      try {
        ...
        // Sort descending by placedAt/createdAt
        fetchedList.sort(
          (a, b) =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-stone-200 p-5 shadow-2xs">
        ...
        {/* Quick Order Status Metrics Bar */}
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => {
            ...
            const isPickup = !!(order.isSelfPickup || order.orderType === 'pickup');
            const targetId = order.orderId || order.id;
            const isExpanded = !!expandedOrderIds[targetId];
            ...
            return (
              ...
              >
                {/* Order Top Header */}
                <div className="p-4 sm:p-5 bg-stone-50/80 border-b border-stone-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-black text-stone-900 font-mono tracking-tight">
                ...
                <div className="p-4 sm:p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 bg-stone-50/60 rounded-xl p-3.5 border border-stone-200/80 text-xs">
                    <div className="space-y-1">
                      ...
                      {order.customerDetails?.deliverySlot && (
                        <div className="text-[11px] text-stone-500">
                          Slot: <span className="font-semibold capitalize">{order.customerDetails.deliverySlot}</span>
                        </div>
                      )}
                    </div>
