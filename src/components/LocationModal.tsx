...
import { POPULAR_PIN_SUGGESTIONS } from '../data/outlets';
...
export const LocationModal: React.FC<LocationModalProps> = ({ isOpen, onClose }) => {
  ...
  const popularPins = React.useMemo(() => {
    const list: { pin: string; area: string; outlet: string }[] = [];
    const seenPins = new Set<string>();

    // 1. First add pins from current active delivery zones in database
    if (Array.isArray(allDeliveryZones) && allDeliveryZones.length > 0) {
      for (const z of allDeliveryZones) {
        ...
        for (const p of z.pinCodes || []) {
          ...
          if (cleanP && !seenPins.has(cleanP)) {
            seenPins.add(cleanP);
            list.push({
              pin: cleanP,
              area: z.name || cityArea,
              outlet: outletName,
            });
          }
        }
    ...
    // 2. Add predefined suggestions if not already added
    for (const sug of POPULAR_PIN_SUGGESTIONS) {
      if (!seenPins.has(sug.pin)) {
        seenPins.add(sug.pin);
        list.push(sug);
      }
    }

    return list.slice(0, 10);
  }, [allDeliveryZones, outlets]);
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-5 py-3 bg-amber-800 hover:bg-amber-900 disabled:bg-stone-300 text-white font-medium text-sm rounded-xl transition-all shadow-sm flex items-center gap-2 cursor-pointer"
                  >
                    {isChecking ? (
              ...
              {successInfo && (
                ...
                >
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-emerald-700 hover:bg-emerald-800 text-white font-medium text-sm rounded-xl transition-all shadow-sm flex items-center justify-center gap-2 cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                  {popularPins.map((item) => (
                    <button
                      ...
                      className="p-2.5 bg-stone-50 hover:bg-amber-50 hover:border-amber-300 border border-stone-200 rounded-xl text-left transition-all flex flex-col group cursor-pointer"
                    >
                      <div className="flex items-center justify-between">
